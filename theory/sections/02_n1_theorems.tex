...
Layer~I.a is not unconditional: its hypothesis A5-under-fault is exactly the
uniqueness that Remark~\ref{rem:bifurcation} warns can break. A bilateral
$\symgrp$-fixed fault deep enough to destabilize the symmetric attractor of
the faulted loop drives it (generically) to a \emph{chiral} limit cycle whose
mirror image is a second attractor: then A1--A4 still hold, the fault is still
$\symgrp$-fixed, yet $\push{\rho}P_\varphi \neq P_\varphi$ and (I.b) makes it
visible. So the blindness has an \emph{amplitude ceiling}: mild-to-moderate
symmetric degradation is invisible to $R^-$, but a sufficiently severe
bilateral fault reappears through the chirality channel $\eps{chir}$
(Assumption~A5$'$). The ceiling is the fault magnitude at which the symmetric
periodic orbit of the faulted plant loses stability; below it the two-channel
design (Corollary~\ref{cor:two-channel-necessity}) is required, above it a
one-sided residual re-emerges. This prediction is tested in the pre-registered
chirality-ceiling experiment (P1, \texttt{experiments/e15\_predictions}): a
bilateral mirror fault of equal severity $\kappa$ is swept downward and $R^-$
power together with the duty-factor chirality index is recorded. The Block~P
run found $R^-$ blind and the chirality index at its floor across the whole
swept range (the symmetric gain fault preserves A5-under-fault and does not
bifurcate at speed~$0$): the blindness is robust and the ceiling is a
phenomenon of a genuine symmetry-breaking regime (the Sprint-1 moving-trot
low-gain chiral gait) rather than of symmetric degradation --- see the P1
anchor below.
\end{remark}
