...
chirality-ceiling experiment (P1, \S\ref{sec:p1-open} anchor;
\texttt{experiments/e15\_predictions}): a bilateral mirror fault of equal
severity $\kappa$ is swept downward and $R^-$ power together with the
duty-factor chirality index is recorded --- the theorem predicts $R^-$ power
$\approx \alpha$ at mild $\kappa$ and a rise coincident with the onset of a
nonzero chirality index at the ceiling.
\end{remark}
...
\emph{Layer~I (law level), pre-registered:} the amplitude-ceiling claim
(Remark~\ref{rem:amplitude-ceiling}) and the statistic-split claim
(Remark~\ref{rem:statistic-consistency}) are tested in
\texttt{experiments/e15\_predictions} (P1, P2). \textbf{[P1/P2 empirical
anchors --- fill after the Block~P runs: P1 $R^-$ power and chirality index
vs bilateral severity $\kappa$ with the ceiling estimate; P2
\texttt{paired\_energy} vs \texttt{energy\_distance} power under a zero-mean
one-sided variance inflation.]} Command conditions that break the symmetry of
the gait itself (turning) are excluded from every blindness claim.
