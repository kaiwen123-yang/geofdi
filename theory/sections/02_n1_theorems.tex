...
plans) and N1-3 (power lower bound, Proposition~\ref{prop:n1-3-power}), with
N1-4 (metric refinement) and the accumulation question left as the open list of
Section~\ref{sec:p1-open}.
...
\subsection{Theorem N1-2: two-layer isotypic detectability (law-level blindness + mean-level power)}
\label{sec:n1-2}
...
The statement is organized in two layers: a \emph{law-level} dichotomy
(layer~I) that says exactly which faults are invisible to \emph{every}
invariance test, and a \emph{mean-level} power characterization (layer~II)
for the specific paired statistic used in practice. The two layers do not
coincide --- Remark~\ref{rem:statistic-consistency} exhibits a fault that is
law-level detectable but mean-level blind --- which is why deployment uses
two statistic families, not one.

\begin{theorem}[N1-2, two-layer isotypic detectability]
\label{thm:n1-2}
\emph{Layer~I (law level).}
\begin{enumerate}
\item[(I.a)] \emph{(blindness)} Suppose the fault pattern is fixed by
$\symgrp$ (stabilizer $\symgrp_\varphi = \symgrp$: bilateral, synchronized,
equal magnitude $\varphi = (\kappa,\kappa)$ on a mirror pair, symmetric wear)
\emph{and} the faulted closed loop still satisfies Assumption~\ref{as:erg}
under the $\symgrp$-symmetric command --- call this \emph{A5-under-fault}: the
faulted plant has a unique invariant measure. Then $\push{\rho}P_\varphi =
P_\varphi$, $H_0$ holds under $P_\varphi$, and every $\symgrp$-invariance
test has power $\le$ its level, whatever the magnitude. Equivalently
$\Pi^-\mu(\kappa,\kappa) = 0$ \emph{and} all higher moments are
$\rho$-invariant.
\item[(I.b)] \emph{(detectability, converse)} Conversely, if $\push{\rho}
P_\varphi \neq P_\varphi$ (any law-level breaking of the invariance, not only
of the mean) then some bounded $h$ has $\mathbb{E}_{P_\varphi}[h(Z) -
h(\rho Z)] \neq 0$, and a statistic that is consistent for the full
two-sample problem between $\{Z_k\}$ and $\{\rho Z_k\}$ --- the
\emph{energy-distance} class $\mathcal{E}(\{Z_k\},\{\rho Z_k\})$
\cite{szekely2013energy}, or any characteristic-kernel MMD --- detects it
with power $\to 1$. Membership of $P_\varphi$ in the blind set of (I.a) is
thus exactly $\push{\rho}P_\varphi = P_\varphi$.
\end{enumerate}
\emph{Layer~II (mean level; the paired-energy flip test).}
\begin{enumerate}
\item[(II.a)] \emph{(noncentrality)} For the paired-energy flip statistic the
noncentrality after $K$ elements is $K\,\|\Pi^-\mu(\varphi)\|^2_{\Sigma}$
(standardized units); the test is consistent \emph{iff} $\Pi^-\mu(\varphi)
\neq 0$ (location alternatives with $\rho$-invariant fluctuations, the
Sprint-6 core). It is therefore consistent against a strict subset of the
laws that (I.b) covers.
\item[(II.b)] \emph{(nonlinear response; the difference heuristic is wrong)}
...
\begin{proof}
\emph{(I.a)} A fault with $\symgrp_\varphi = \symgrp$ maps the closed loop to
itself under every $\sigma \in \symgrp$: the faulted transition kernels are
conjugation-invariant exactly as in A1--A4 (the fault is part of the
equivariant plant). By Remark~\ref{rem:bifurcation} equivariance alone gives
$\push{\rho}\mu$ invariant whenever $\mu$ is; A5-under-fault (uniqueness for
the faulted loop) is what forces $\push{\rho}P_\varphi = P_\varphi$, i.e.\
\eqref{eq:h0} for $P_\varphi$. Theorem~\ref{thm:n1-1} then makes every flip
test level-$\alpha$ under $P_\varphi$. \emph{(I.b)} If $\push{\rho}P_\varphi
\neq P_\varphi$ two probability measures differ, so some bounded continuous
$h$ separates them, $\mathbb{E}_{P_\varphi}[h(Z)] \neq \mathbb{E}_{P_\varphi}
[h(\rho Z)]$; the energy distance (equivalently the MMD with a characteristic
kernel) is zero iff the two laws coincide and is estimated $\sqrt K$-consistently
from the paired sample, so the permutation flip test built on it has power
$\to 1$ \cite{szekely2013energy}. \emph{(II.a)} With $\push{\rho}P_\varphi$
allowed to differ from $P_\varphi$ only in the mean, the noncentrality is the
Sprint-6 core: $D_k = 2\Pi^-\mu + (\xi_k - \rho\xi_k)$, $\mathbb{E}\|\bar
D\|^2 = 4\|\Pi^-\mu\|^2 + \operatorname{tr}\operatorname{Var}(D_1)/K$ against
a flip-null expectation of $\operatorname{tr}\operatorname{Var}(D_1)/K +
O(\|\Pi^-\mu\|^2/K)$; consistency follows from the law of large numbers on
both, and holds iff $\Pi^-\mu \neq 0$. Since $\Pi^-\mu = 0$ is possible with
$\push{\rho}P_\varphi \neq P_\varphi$ (equal means, unequal higher moments;
Remark~\ref{rem:statistic-consistency}), (II.a) covers a strict subset of
(I.b). \emph{(II.b)} In leg-pair coordinates $\mu = (f(\kappa_{\mathrm{L}}),
...
\begin{remark}[Amplitude ceiling of the blindness --- A5-under-fault can fail]
\label{rem:amplitude-ceiling}
\emph{What was measured first.} The pre-registered chirality-ceiling experiment
(P1, \texttt{experiments/e15\_predictions}, $R = 30$ per point) swept a
bilaterally symmetric actuator-gain fault of equal severity on a mirror pair
over $1-\kappa \in \{0.3, 0.5, 0.6, 0.7\}$ --- i.e.\ down to $30\,\%$ of nominal
torque, far past any operating point one would call a degradation --- and found
$R^-$ power $0.00$ at \emph{every} severity, with the realized-gait chirality
index (the mirror residual of the stance duty factors and touch-down phases,
the $\eps{chir}$ of A5$'$) pinned at its nominal floor $\approx 0.003$
throughout. \textbf{The blindness of Layer~I.a is therefore robust to
amplitude: symmetric degradation, however severe, does not by itself break the
symmetric attractor.} The pre-registered expectation of a ceiling inside this
range was \emph{not} met, and the statement below is the corrected one.

\emph{What the ceiling actually requires.} Layer~I.a is still not
unconditional, but the escape route is narrower than "large enough magnitude".
Its hypothesis A5-under-fault is the uniqueness that
Remark~\ref{rem:bifurcation} warns can break, and breaking it needs the faulted
loop to enter a genuine \emph{symmetry-breaking bifurcation}, not merely a large
fault: the symmetric periodic orbit must lose stability, after which an
equivariant system settles on a \emph{chiral} limit cycle whose mirror image is a
second attractor. Then A1--A4 hold, the fault is still $\symgrp$-fixed, yet
$\push{\rho}P_\varphi \neq P_\varphi$ and (I.b) makes it visible through
$\eps{chir}$. Such a regime does exist in this project's own worlds --- the
Sprint-1 moving-trot Go2 at low proportional gain realized a chiral gait with
duty factors $0.63/0.64$ and $0.85/0.87$ and a chirality index $\approx 0.23$,
two orders of magnitude above the P1 floor (rp003, Finding~1) --- but it is
reached by \emph{destabilizing the gait}, not by scaling a symmetric fault at a
stable operating point. The practical reading is therefore stronger than the
original conjecture: within the envelope where the commanded gait remains
stable, the two-channel design
(Corollary~\ref{cor:two-channel-necessity}) is \emph{necessary}, and one should
not count on a severe bilateral fault eventually revealing itself to $R^-$.
Falsification of the corrected statement: a $\symgrp$-fixed fault that raises
$R^-$ power above $\alpha$ while the chirality index stays at its nominal floor
(that would break Layer~I.a itself), or a bifurcating regime in which $R^-$
stays blind although the chirality index has risen (that would break the
$\eps{chir}$ route).
\end{remark}

\begin{remark}[Statistic consistency --- paired mean vs energy distance]
\label{rem:statistic-consistency}
Layers~I and~II differ precisely on \emph{zero-mean law breaking}. A fault
that changes the law of a mirror pair without changing its mean --- e.g.\ a
one-sided variance inflation, the left leg's encoder-noise variance raised
while every mean profile stays $\rho$-fixed --- has $\Pi^-\mu = 0$ but
$\push{\rho}P_\varphi \neq P_\varphi$. By (II.a) the paired-energy /
paired-mean flip test has power $\to \alpha$ (blind); by (I.b) the
energy-distance flip test has power $\to 1$ (visible). Neither dominates: the
paired-mean statistic is more powerful against the location alternatives that
dominate the actuator-fault families (II.a noncentrality $K\|\Pi^-\mu\|^2$),
the energy-distance statistic is the one with the full converse. A deployment
should therefore run \emph{both} families on the same elements and alarm on
either. This is the pre-registered statistic-split experiment (P2,
\texttt{experiments/e15\_predictions}): left-leg encoder variance $\times 4$
at zero mean, with the prediction \texttt{paired\_energy} power $\approx
\alpha$ and \texttt{energy\_distance} power $> \alpha$.
\end{remark}
...
\emph{Layer~II (mean level).} e04c (rp005; the isotypic figure and the
projection-energy table of that pack): single $R^-$ $1.00$ / $R^+$ $1.00$;
bilateral equal $R^-$ alarm $0.00$ (blind), $R^+$ $1.00$; unequal $(0.7, 0.5)$:
$R^-$ $1.00$ with \emph{more} antisymmetric energy than the single ($87.4$ vs
$60.8$) --- the pre-registered ``strictly between'' prediction was falsified,
the share ordering $0.554 > 0.201 > 0.007$ held: (II.b) is the corrected
statement. e13c on the equivariant residual: shares $0.49 / 0.000 / 0.15$ (a
one-joint footprint splits exactly $\tfrac12$--$\tfrac12$ on the residual;
$0.43$ on the raw element because kinematic coupling spreads the footprint),
bilateral-equal $R^-$ e-process $0.00$, $\Pi^+$-magnitude $1.00$. e03
(external Gazebo benchmark, Block~E): the mirror-double class is a blindness
test on data we did not generate --- reported \emph{inconclusive} at $n = 8$
straight episodes ($R^-$ straight $0.50$; too few to separate ``blind'' from
``underpowered'', \texttt{docs/protocol/liu\_a1\_audit.md} \S4). Layer~II is
falsified if a $\symgrp$-fixed, A5-under-fault fault is detected by the
paired flip test above its false-alarm rate on a world satisfying A1--A5.
\emph{Layer~I (law level).} P1 (\texttt{experiments/e15\_predictions}, stage~a;
\texttt{e15a\_chirality\_ceiling}): a bilaterally-symmetric actuator-gain fault
(equal $\kappa$ on the front mirror pair) on \texttt{go2\_urdf\_sym},
$1-\kappa \in \{0.3,0.5,0.6,0.7\}$, keeps $R^-$ power at $0.00$ (blind) at
\emph{every} severity, with the realized-gait chirality index (stance duty
and touch-down mirror residual) at its $\approx 0.00$ floor throughout ---
A5-under-fault holds and no bifurcation occurs, so the blindness is robust and
\emph{the amplitude ceiling is not reached by a symmetric gain fault in this
speed-$0$ regime} (the chirality bifurcation is the Sprint-1 moving-trot
low-gain phenomenon, rp003, chirality $\approx 0.23$; a symmetric gain
reduction here degrades symmetrically instead of breaking chirally). This
confirms Layer~I.a (blindness under A5-under-fault) and leaves the ceiling as
a predicted phenomenon of a genuine symmetry-breaking regime, not of symmetric
degradation. P2 (stage~b; \texttt{e15b\_statistic\_split}): a \emph{zero-mean}
variance inflation ($\Pi^-\mu = 0$) --- in the controlled toy with the exact
hypotheses (an i.i.d.\ Gaussian mirror pair, LF/RF variance ratio $1,2,4,9$)
\texttt{paired\_energy} power stays at $\alpha$ ($0.05,0.10,0.03,0.03$: blind,
Layer~II.a) while \texttt{energy\_distance} power rises to $0.88$ (consistent,
Layer~I.b); in the closed loop the same direction holds near the flip-test
floor (a single channel's scale change, standardized across the $50$-channel
element, is diluted, and energy distance has intrinsically low power for pure
scale alternatives at $K \le 80$). \emph{Deployment consequence
(\texttt{protocol\_params.md}): run both statistic families and alarm on
either --- \texttt{paired\_energy} alone misses a zero-mean/variance-only
fault.} Command conditions that break the symmetry of the gait itself
(turning) are excluded from every blindness claim.
...
One item on this list is closed by Sprint~9 and is stated first; the rest remain
open.

\begin{proposition}[N1-3: power lower bound for the quadratic flip statistic]
\label{prop:n1-3-power}
Model the differenced half-cycle scores as Gaussian: $D_k = Z_k - \rho Z_k$ with
$D_k \sim \mathcal{N}(2\Pi^-\mu, \Sigma_D)$ i.i.d.\ across the $K$ elements, and
let $T = K\,\|\bar D\|^2_{\Sigma_D}$ be the whitened quadratic flip statistic on
$d = \dim \mathcal{W}^-$ effective coordinates, compared with the level-$\alpha$
critical value $c_\alpha = \chi^2_{d,1-\alpha}$. Write the noncentrality
\begin{equation}
  \label{eq:n13-lambda}
  \lambda \;=\; K\,\|\Pi^-\mu\|^2_{\Sigma_D}.
\end{equation}
Then the power satisfies the explicit lower bound
\begin{equation}
  \label{eq:n13-bound}
  \mathbb{P}(T > c_\alpha)
  \;\ge\; 1 - \exp\!\Bigl(-\frac{(d + \lambda - c_\alpha)_+^2}{4\,(d + 2\lambda)}\Bigr).
\end{equation}
Consequently, for a target power $1-\beta$ it suffices that
$\lambda \ge \lambda^*(d,\alpha,\beta)$ with
\begin{equation}
  \label{eq:n13-samplesize}
  \sqrt{\lambda^*} \;=\; \sqrt{2\ln\tfrac1\beta}
  \;+\; \sqrt{\,2\ln\tfrac1\beta + 2\sqrt{d\ln\tfrac1\alpha} + 2\ln\tfrac1\alpha + 2\sqrt{d\ln\tfrac1\beta}\,},
\end{equation}
i.e.\ the number of cycles needed is
$K \ge \lambda^*(d,\alpha,\beta)\,/\,\|\Pi^-\mu\|^2_{\Sigma_D}$ --- the
sample-size rule that Theorem~\ref{thm:n1-2}(II.a) left implicit.
\end{proposition}

\begin{proof}
Under the stated model $T \sim \chi^2_d(\lambda)$ with $\lambda$ as in
\eqref{eq:n13-lambda}. Laurent--Massart's lower-tail inequality for a
noncentral chi-square \cite{laurent2000adaptive} gives, for every $x > 0$,
$\mathbb{P}\bigl(T \le d + \lambda - 2\sqrt{(d + 2\lambda)x}\bigr) \le e^{-x}$.
Choosing $x$ so that the left endpoint equals $c_\alpha$, i.e.
$x = (d+\lambda-c_\alpha)^2 / \bigl(4(d+2\lambda)\bigr)$ whenever
$d + \lambda > c_\alpha$, yields $\mathbb{P}(T \le c_\alpha) \le e^{-x}$, which
is \eqref{eq:n13-bound}. For \eqref{eq:n13-samplesize}, power $\ge 1-\beta$
holds as soon as $x \ge \ln(1/\beta)$, i.e.
$d + \lambda - c_\alpha \ge 2\sqrt{(d+2\lambda)\ln(1/\beta)}$. Bound the
critical value by the companion upper tail,
$c_\alpha \le d + 2\sqrt{d\ln(1/\alpha)} + 2\ln(1/\alpha)$, and
$\sqrt{d + 2\lambda} \le \sqrt d + \sqrt{2\lambda}$; the requirement becomes
$\lambda - 2\sqrt{2\ln(1/\beta)}\sqrt{\lambda} - A \ge 0$ with
$A = 2\sqrt{d\ln(1/\alpha)} + 2\ln(1/\alpha) + 2\sqrt{d\ln(1/\beta)}$, a
quadratic in $\sqrt\lambda$ whose positive root is \eqref{eq:n13-samplesize}.
\end{proof}

\paragraph{Falsification, tightness and anchor.}
The bound is falsified by any $(d,\lambda,\alpha)$ with true power below
\eqref{eq:n13-bound}; a numerical sweep over $d \in \{4,12,40,100\}$,
$\lambda \in [0,120]$ and $\alpha \in \{0.01,0.05\}$ found none, and the
sample-size rule \eqref{eq:n13-samplesize} delivered at least the requested
power in every case checked. Tightness: at $d = 12$ the true power reaches
$0.8$ at $\lambda = 17.4$ while the bound reaches it at $\lambda = 30.7$, a
factor $1.76$ in $\lambda$ and hence only $\approx 1.33$ in the fault
\emph{magnitude} (since $\lambda \propto \|\Pi^-\mu\|^2$) --- usable for
sizing an experiment, not for predicting a knee. The Gaussian premise is the
limitation: the deployed statistic is a permutation-calibrated energy
statistic, not a whitened quadratic form, so \eqref{eq:n13-bound} describes
its Gaussian surrogate; the e04a/e08 power curves remain the empirical
reference.
