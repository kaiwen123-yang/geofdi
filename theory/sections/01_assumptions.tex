...
which justifies the identification $\epsbar{env} = \eps{env}$.
\end{definition}
...
$\mathbb{E}[\varphi] \le \alpha + \epsbar{tot}$ under the actual nominal law.
\end{lemma}
...
$\dtv$, $\dtv(P, \widetilde P) \le \max_{\sigma} \dtv(P, \push{\rho(\sigma)}P)
\le \epsbar{tot}$; hence $\mathbb{E}_P[\varphi] \le
\mathbb{E}_{\widetilde P}[\varphi] + \epsbar{tot} \le \alpha + \epsbar{tot}$.
...
at phase $\Delta\theta$), and the tightening of $\epsbar{\bullet}$ discussed
in Remark~\ref{rem:linear} --- is deferred to workstream N1-4.
\end{proof}
...
\end{equation}
This linear bound is pessimistic: it charges every step's defect at full
weight for the remainder of the cycle. If the one-step closed-loop state
kernel (conditionally on the environment draw) is a Dobrushin contraction
with coefficient $\delta < 1$ uniformly along the cycle, the perturbation
injected at step $t$ is damped by $\delta^{\,s-t}$ at step $s$, so the defect
visible in the state marginal at any fixed phase accumulates as a geometric
sum, $\sum_{t \le s} \delta^{\,s-t} \eps{\bullet} \le \eps{\bullet}/(1-\delta)$,
and one expects
\[
  \epsbar{\bullet} \;\lesssim\; \frac{\eps{\bullet}}{1-\delta}
\]
--- of the order of the per-transition defect rather than $\Tc$ times it.
Making this precise for the trajectory-level quantity
\eqref{eq:cycle-eps} (which functionals of $Z_k$ inherit the geometric
rate, and how the phase-grid resolution $N$ enters) is deferred to
workstream N1-4; until then \eqref{eq:linear-bound} is the operative
conversion.
\end{remark}
...
Remark~\ref{rem:linear}, or by the tighter contraction-based conversion once
workstream N1-4 delivers it. The unconditional mirrored-command distance of
the A2 audit already lives on the closed-loop scale and needs no lifting.
