...
mean. Then the centred profiles $\tilde R_k := R_k - \hat{\bar m}$ satisfy:
(i) under nominal operation $\mathbb{E}\tilde R_k = 0$ and
$\operatorname{Var}(\tilde R_k) = \Sigma_\Xi (1 + K_{\mathrm{cal}}^{-1})$, so
$\beta_{\mathrm{op}}^2 = \lambda_{\max}(\Sigma_\Xi)(1+K_{\mathrm{cal}}^{-1})$
does not depend on $\bar m$; (ii) under the fault the population second
moment of $\tilde R_k$ is $m^2 s_c s_c^\top + \Sigma_\Xi(1+K_{\mathrm{cal}}^{-1})$,
which is the certificate's setting with $\beta_{\mathrm{op}}^2$ as in (i)
and the fault signature intact; (iii) for the $R^{-}$ channel, centring
removes the \emph{first-order} contamination of Corollary~\ref{cor:contamination}:
$\Pi^{-}\tilde R_k = \Pi^{-}\Xi_k - \Pi^{-}(\hat{\bar m} - \bar m)$ has mean
zero even if $\Pi^{-}\bar m \neq 0$ (non-equivariant $\hat f$), but the flip
test on $\tilde R_k$ is exact only if $\Law(\rho_R \Xi_k) = \Law(\Xi_k)$
--- second-order asymmetries of a non-equivariant model (mirror-unequal
variances of the fluctuation) survive centring --- and the estimation error
$\hat{\bar m} - \bar m = O_p(K_{\mathrm{cal}}^{-1/2})$ is a common offset of
all monitored cycles, so the size after centring is nominal only for
$K \ll K_{\mathrm{cal}}$.
\end{lemma}
...
identically and the variances add. (iii): $\rho_R \tilde R_k \eqdist \tilde R_k$
requires $\Law(\rho_R\Xi_k - \rho_R\bar\Xi_{\mathrm{cal}}) = \Law(\Xi_k -
\bar\Xi_{\mathrm{cal}})$, which holds iff the fluctuation law is invariant;
a common offset $-\Pi^-\bar\Xi_{\mathrm{cal}}$ of variance
$\Sigma^-/K_{\mathrm{cal}}$ added to all $K$ paired differences enters the
paired-energy statistic as a mean shift of that size, i.e.\ a noncentrality
of order $K/K_{\mathrm{cal}}$ in standardized units.
\end{proof}

This is the mechanism behind Sprint~4, Finding~4 (degrading a DeLaN by
sample size raised $\hat\beta$ five-fold but $\beta_{\mathrm{op}}$ only by
$1.7\times$: the systematic part was calibrated away), and it is Part~0's
$H_0'$ route applied to residuals: centring is the calibration of
$\nu_0$. Its limits --- second-order asymmetry, and the $K/K_{\mathrm{cal}}$
term --- are why the equivariant model of the next proposition is preferable
to centring a non-equivariant one for the $R^{-}$ channel: with
$\delta_f = 0$ nothing needs to be calibrated away.
...
(2) calibration-centre the residual (Lemma~\ref{lem:centring}): first-order
removal, $H_0'$ on residuals; (3) budget it: level
...
\pending{back-fill from e13c} \\
Prop.~\ref{prop:equiv-delan} &
Block~Q: $\delta_f^{(0.95)}$ and validation RMSE, plain vs equivariant
ladders &
\pending{back-fill from Block~Q} \\
\bottomrule
\end{tabular}
\caption{Empirical anchors of Part~2. Entries are filled from the e13 run
and the Block~Q reports; wheeled-M1 anchors (e13d) are pending the M1
world.}
\label{tab:anchors}
\end{table}
